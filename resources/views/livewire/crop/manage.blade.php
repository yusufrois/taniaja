<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold">🌾 Komoditas (Crop)</h1>
            <p class="lw-muted text-sm">Kelola daftar komoditas yang ditanam.</p>
        </div>
        @if ($canCreate)
            <button wire:click="openCreate" class="btn-primary px-4 py-2 text-sm">+ Tambah Komoditas</button>
        @endif
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left lw-muted text-xs uppercase tracking-wide" style="border-bottom:1px solid rgba(148,163,184,.15)">
                        ...
                        <th class="px-4 py-3">Jumlah Varietas</th>
                ...
                <tbody>
                    @forelse ($crops as $crop)
                        <tr style="border-bottom:1px solid rgba(148,163,184,.08)">
                            ...
                            <td class="px-4 py-3 lw-muted">{{ $crop->varieties_count }}</td>
                            <td class="px-4 py-3 lw-muted">{{ $crop->notes ?: '-' }}</td>
                            <td class="px-4 py-3 text-right space-x-2">
                                <a href="{{ route('web.varieties', ['crop_id' => $crop->id]) }}" class="btn-secondary px-3 py-1.5 text-xs">Varietas</a>
                                @can('update', $crop)
                                    <button wire:click="openEdit({{ $crop->id }})" class="btn-secondary px-3 py-1.5 text-xs">Edit</button>
                                @endcan
                                @can('delete', $crop)
                                    <button wire:click="confirmDelete({{ $crop->id }})" class="btn-secondary px-3 py-1.5 text-xs" style="color:#fb7185">Hapus</button>
                                @endcan
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="4" class="px-4 py-10 text-center lw-muted">Belum ada data komoditas.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if ($crops->hasPages())
            <div class="px-4 py-3" style="border-top:1px solid rgba(148,163,184,.08)">{{ $crops->links() }}</div>
        @endif
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 overflow-y-auto px-4" style="z-index:100; background-color: rgba(0,0,0,0.88)" wire:click.self="closeModal">
            <div class="mx-auto my-8 w-full max-w-md p-6" style="background: rgba(8,20,14,0.98); border: 1px solid rgba(74,222,128,.15); border-radius: 16px; box-shadow: 0 12px 35px rgba(0,0,0,.35);">
                <h2 class="text-lg font-bold mb-4">{{ $editingId ? 'Edit Komoditas' : 'Tambah Komoditas' }}</h2>
                ...
                <form wire:submit="save" class="space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model="name" class="lw-input w-full px-3" placeholder="Melon">
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 overflow-y-auto px-4" style="z-index:100; background-color: rgba(0,0,0,0.88)" wire:click.self="$set('confirmingDeleteId', null)">
            <div class="mx-auto my-8 w-full max-w-sm p-6 text-center" style="background: rgba(8,20,14,0.98); border: 1px solid rgba(74,222,128,.15); border-radius: 16px; box-shadow: 0 12px 35px rgba(0,0,0,.35);">
                ...
                @endif
                <p class="mb-4">Yakin ingin menghapus komoditas ini?</p>
