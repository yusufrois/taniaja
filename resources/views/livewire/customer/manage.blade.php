<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="lw-muted text-sm">Kelola daftar pelanggan/pembeli.</p>
        </div>
        @if ($canCreate)
            <button wire:click="openCreate" class="btn-primary px-4 py-2 text-sm">+ Tambah Customer</button>
        @endif
    </div>
                ...
                <tbody>
                    @forelse ($customers as $customer)
                        <tr style="border-bottom:1px solid rgba(148,163,184,.08)">
                            ...
                            <td class="px-4 py-3 lw-muted">{{ $customer->type === 'business' ? 'Badan Usaha' : 'Perorangan' }}</td>
                            <td class="px-4 py-3 lw-muted">{{ $customer->phone ?: '-' }}</td>
                            <td class="px-4 py-3 lw-muted">{{ $customer->address ?: '-' }}</td>
                            <td class="px-4 py-3 text-right space-x-2">
                                @can('update', $customer)
                                    <button wire:click="openEdit({{ $customer->id }})" class="btn-secondary px-3 py-1.5 text-xs">Edit</button>
                                @endcan
                                @can('delete', $customer)
                                    <button wire:click="confirmDelete({{ $customer->id }})" class="btn-secondary px-3 py-1.5 text-xs" style="color:#fb7185">Hapus</button>
                                @endcan
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="px-4 py-10 text-center lw-muted">Belum ada data customer.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if ($customers->hasPages())
            <div class="px-4 py-3" style="border-top:1px solid rgba(148,163,184,.08)">{{ $customers->links() }}</div>
        @endif
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 overflow-y-auto px-4" style="z-index:100; background-color: rgba(0,0,0,0.88)" wire:click.self="closeModal">
            <div class="mx-auto my-8 w-full max-w-lg p-6" style="background: rgba(8,20,14,0.98); border: 1px solid rgba(74,222,128,.15); border-radius: 16px; box-shadow: 0 12px 35px rgba(0,0,0,.35);">
                <h2 class="text-lg font-bold mb-4">{{ $editingId ? 'Edit Customer' : 'Tambah Customer' }}</h2>
                ...
                <form wire:submit="save" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" wire:model="name" class="lw-input w-full px-3" placeholder="Toko Buah Segar">
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 overflow-y-auto px-4" style="z-index:100; background-color: rgba(0,0,0,0.88)" wire:click.self="$set('confirmingDeleteId', null)">
            <div class="mx-auto my-8 w-full max-w-sm p-6 text-center" style="background: rgba(8,20,14,0.98); border: 1px solid rgba(74,222,128,.15); border-radius: 16px; box-shadow: 0 12px 35px rgba(0,0,0,.35);">
                ...
                @endif
                <p class="mb-4">Yakin ingin menghapus customer ini?</p>
