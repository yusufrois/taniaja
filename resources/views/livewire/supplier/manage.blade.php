<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold">🚜 Supplier / Petani</h1>
            <p class="lw-muted text-sm">Kelola daftar petani/tengkulak sumber pembelian.</p>
        </div>
        @if ($canCreate)
            <button wire:click="openCreate" class="btn-primary px-4 py-2 text-sm">+ Tambah Supplier</button>
        @endif
    </div>
                ...
                <tbody>
                    @forelse ($suppliers as $supplier)
                        <tr style="border-bottom:1px solid rgba(148,163,184,.08)">
                            ...
                            <td class="px-4 py-3 lw-muted">{{ $supplier->phone ?: '-' }}</td>
                            <td class="px-4 py-3 lw-muted">{{ $supplier->email ?: '-' }}</td>
                            <td class="px-4 py-3 lw-muted">{{ $supplier->address ?: '-' }}</td>
                            <td class="px-4 py-3 text-right space-x-2">
                                @can('update', $supplier)
                                    <button wire:click="openEdit({{ $supplier->id }})" class="btn-secondary px-3 py-1.5 text-xs">Edit</button>
                                @endcan
                                @can('delete', $supplier)
                                    <button wire:click="confirmDelete({{ $supplier->id }})" class="btn-secondary px-3 py-1.5 text-xs" style="color:#fb7185">Hapus</button>
                                @endcan
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="px-4 py-10 text-center lw-muted">Belum ada data supplier.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if ($suppliers->hasPages())
            <div class="px-4 py-3" style="border-top:1px solid rgba(148,163,184,.08)">{{ $suppliers->links() }}</div>
        @endif
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 overflow-y-auto px-4" style="z-index:100; background-color: rgba(0,0,0,0.88)" wire:click.self="closeModal">
            <div class="mx-auto my-8 w-full max-w-lg p-6" style="background: rgba(8,20,14,0.98); border: 1px solid rgba(74,222,128,.15); border-radius: 16px; box-shadow: 0 12px 35px rgba(0,0,0,.35);">
                <h2 class="text-lg font-bold mb-4">{{ $editingId ? 'Edit Supplier' : 'Tambah Supplier' }}</h2>
                ...
                <form wire:submit="save" class="space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model="name" class="lw-input w-full px-3" placeholder="Pak Tani Slamet">
                        @error('name') <p class="lw-error text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="lw-label">Telepon</label>
                            <input type="text" wire:model="phone" class="lw-input w-full px-3">
                        </div>
                        <div>
                            <label class="lw-label">Email</label>
                            <input type="email" wire:model="email" class="lw-input w-full px-3">
                            @error('email') <p class="lw-error text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div>
                        <label class="lw-label">Alamat</label>
                        <input type="text" wire:model="address" class="lw-input w-full px-3">
                    </div>
                    <div>
    ...
    @if ($confirmingDeleteId)
        <div class="fixed inset-0 overflow-y-auto px-4" style="z-index:100; background-color: rgba(0,0,0,0.88)" wire:click.self="$set('confirmingDeleteId', null)">
            <div class="mx-auto my-8 w-full max-w-sm p-6 text-center" style="background: rgba(8,20,14,0.98); border: 1px solid rgba(74,222,128,.15); border-radius: 16px; box-shadow: 0 12px 35px rgba(0,0,0,.35);">
                ...
                @endif
                <p class="mb-4">Yakin ingin menghapus supplier ini?</p>
