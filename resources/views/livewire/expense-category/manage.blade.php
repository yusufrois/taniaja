<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold">📁 Kategori Beban</h1>
            <p class="lw-muted text-sm">Kelola kategori untuk pencatatan pengeluaran.</p>
        </div>
        @if ($canCreate)
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="2" class="px-4 py-10 text-center lw-muted">Belum ada data kategori.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 overflow-y-auto px-4" style="z-index:100; background-color: rgba(0,0,0,0.88)" wire:click.self="closeModal">
            <div class="mx-auto my-8 w-full max-w-sm p-6" style="background: rgba(8,20,14,0.98); border: 1px solid rgba(74,222,128,.15); border-radius: 16px; box-shadow: 0 12px 35px rgba(0,0,0,.35);">
                <h2 class="text-lg font-bold mb-4">{{ $editingId ? 'Edit Kategori' : 'Tambah Kategori' }}</h2>
                ...
                <form wire:submit="save" class="space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model="name" class="lw-input w-full px-3" placeholder="Pupuk & Nutrisi">
